...
/// into <see cref="LastError"/>. A messaging railway (Wolverine's, for instance) refuses <i>without</i>
...
/// the bus's own log stream, never as data a spec could assert on.
/// </para>
/// <para>
...
public abstract class CritterStackFixture : Fixture
{
    ...
    private ActExecution _lastExecution = ActExecution.None;
    ...
    /// scenario's published <see cref="ActExecution"/>, so an act performed by a
    ...
    /// </remarks>
    public ActExecution LastExecution
        => Context != null && Context.TryGetState<ActExecution>(out var published)
            ? published
            : _lastExecution;
    ...
    /// <c>an act a derived fixture adds</c>,
    ...
    /// </summary>
    public void RecordExecution(ActExecution execution)
    {
    ...
    /// <summary>What the last act did, or null when it failed to run.</summary>
    protected IActOutcome? LastOutcome => LastExecution.Outcome;
    ...
    /// <see cref="ActExecution"/>, and the old message discarded it — so the one fact that
    ...
    /// </list>
    /// </remarks>
    private IActOutcome requireOutcome(string expectation)
    {
        var execution = LastExecution;
        if (execution.Outcome != null) return execution.Outcome;

        if (execution.Error != null)
            throw new SpecAssertionException(
                $"{expectation}, but the act failed before it completed — " +
    ...
    /// <summary>The step context, or a clear error when a step runs outside a scenario.</summary>
    protected IStepContext Ctx => Context ?? throw new InvalidOperationException(
    ...
    public void BeforeEach()
    {
        ...
        StreamKey = null;
        AggregateType = null;
        RecordExecution(ActExecution.None);
    }
    ...
    /// <summary>
    /// Act: send <paramref name="command"/> into the application, wait for it to settle,
    ...
    public async Task<AggregateExecution<T>?> WhenCommand<T>(object command) where T : class
    {
        ...
        return new AggregateExecution<T>(LastOutcome!, LastEvents, aggregate);
    }
    ...
    /// <summary>
    /// Assert the last command was cleanly refused — a non-throwing messaging railway (Wolverine's, say), a
    ...
    /// </summary>
    /// <remarks>
    /// Deliberately reason-less: a clean stop carries no reason anywhere the bus surfaces —
    ...
    {
        var outcome = requireOutcome($"Expected {count} message(s) of type {typeof(T).Name} to be sent");

        var sent = outcome.MessagesSent.OfType<T>().Count();
        if (sent < count)
            throw new SpecAssertionException(
    ...
    {
        var outcome = requireOutcome($"Expected a {message.Name} message to be sent");

        var sent = outcome.MessagesSent.Any(m => m.GetType() == message);
        if (!sent)
            throw new SpecAssertionException(
                $"Expected a {message.Name} message to be sent. Sent: {describe(outcome.MessagesSent)}");
    }

    // ---- plumbing -----------------------------------------------------------------------------

    /// <summary>
    /// <b>The one thing this grammar cannot do on its own.</b> Send <paramref name="command"/> into
    /// the application and return when everything it caused has settled — cascades, forwarded
    /// events, local queues drained — reporting the messages it sent.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Run an act through the shared bracket, so its outcome feeds every assertion step
    /// identically. For a derived fixture adding an act of its own.
    /// </summary>
    protected Task ExecuteActAsync(Func<Task<IActOutcome>> dispatch)
        => executeTrackedCore(dispatch);
    ...
    private Task executeCommandCore(object command)
    {
        ...
        return executeTrackedCore(() => DispatchAsync(command, DispatchTimeoutInMilliseconds));
    }

    /// <summary>How long an act may take to settle. Override for a slow application.</summary>
    protected virtual int DispatchTimeoutInMilliseconds => 5000;

    /// <summary>
    /// The shared act bracket, delegated to <see cref="ScenarioActs.ExecuteAsync"/>: snapshot the
    ...
    /// <see cref="WhenCommand{T}"/> and any act a derived fixture adds, so every act feeds the
    /// assertion steps identically.
    /// </summary>
    private async Task executeTrackedCore(Func<Task<IActOutcome>> dispatch)
        => RecordExecution(await ScenarioActs.ExecuteAsync(Ctx, dispatch, streamIdentity, HostResource, StoreName));
