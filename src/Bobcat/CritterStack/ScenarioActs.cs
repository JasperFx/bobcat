...
/// <see cref="Guid"/> or a string stream key). CritterStackFixture's Given steps
/// publish it onto the scenario-state blackboard, so a cooperating grammar performing its own act
/// (an HTTP grammar) can bracket the same stream without sharing a fixture
...
/// <summary>
/// The shared act bracket behind every Critter Stack act step — a dispatched command, an HTTP call, a composed grammar's own act: snapshot the scenario's current stream, run the
/// dispatch, and capture what it did (session, newly appended events, or the exception)
/// as an <see cref="ActExecution"/>, published onto the scenario-state blackboard so every
/// assertion step — whichever grammar declares it — reads the same record.
/// </summary>
public static class ScenarioActs
{
    /// <summary>
    ...
    /// bracketed and <see cref="ActExecution.NewEvents"/> stays empty.
    /// </summary>
    public static async Task<ActExecution> ExecuteAsync(
        IStepContext context,
        Func<Task<IActOutcome>> dispatch,
        ...
        string? storeName = null)
    {
        ...
        try
        {
            ...
            execution = new ActExecution(outcome, appended, null);
            ...
            recordTouched(context, outcome.MessagesSent);
        }
        catch (Exception e)
        {
            execution = new ActExecution(null, [], e);
        }
