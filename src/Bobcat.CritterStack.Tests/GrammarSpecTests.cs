...
/// <see cref="WalletFixture"/> derives from <see cref="WolverineCritterStackFixture"/> and declares nothing.
/// </summary>
/// <remarks>
...
public class GrammarSpecTests
{
    ...
    public async Task the_shipped_grammar_feature_compiles_runs_on_marten_and_renders()
    {
        ...
        {
            var results = await run(feature, scenario, resources);
            ...
            {
                // Name the scenario and step: nine scenarios share this loop, and a bare status
                // mismatch says nothing about which one broke or why.
    ...
    private static async Task<ExecutionResults> run(FeatureDefinition feature, ScenarioDefinition scenario, TestResources resources)
    {
        ...
        var context = new SpecExecutionContext(scenario.Title, resources: resources) { Cancellation = CancellationToken.None };
        fixture.Context = context;

        var resource = resources.GetResource<IHostResource>();
